def processInputs(inputs):
    return sum([max(line) - min(line) for line in inputs])


def getDivisibleInputDiff(rowOfInputs):
    for primaryIndex in range(1, len(rowOfInputs)):
        for secondaryIndex in range(0, primaryIndex):
            elements = [rowOfInputs[primaryIndex], rowOfInputs[secondaryIndex]]

            if max(elements) % min(elements) == 0:
                return max(elements)//min(elements)


def processDivisibleInputs(inputs):
    return sum([getDivisibleInputDiff(line) for line in inputs])
...
def main():
    ...
    print(processInputs(fullInputs))
    print(processDivisibleInputs(fullInputs))
